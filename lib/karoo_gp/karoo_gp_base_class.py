...
class Base_GP(object):
	...
	def karoo_banner(self):
		...
		print('\t **    ** **    **  **    **  ******    ******       ******   **')
		print('')
		print('\tGenetic Programming in Python - by Kai Staats, version 1.0')
		...
		while True:
			try:
				...
				if pause == '?' or pause == 'help':
					...
					print('\t i \t Interactive display mode')
					print('\t m \t Minimal display mode')
					print('\t g \t Generation display mode')
					print('\t s \t Silent display mode')
					print('\t db \t De-Bug display mode')
					print('')
					print('\t ts \t adjust the tournament size')
					print('\t min \t adjust the minimum number of nodes')
					# print '\t max \t adjust the maximum Tree depth'
					print('\t bal \t adjust the balance of genetic operators')
					print('')
					print('\t l \t list Trees with leading fitness scores')
					print('\t t \t evaluate a single Tree against the test data')
					print('')
					print('\t p \t print a single Tree to screen')
					print('\t id \t display the current generation ID')
					print('\t pop \t list all Trees in current population')
					print('\t dir \t display current working directory')
					print('')
					print('\t cont \t continue evolution, starting with the current population')
					print('\t load \t load population_s (seed) to replace population_a (current)')
					print('\t w \t write the evolving population_b to disk')
					print('\t q \t quit Karoo GP without saving population_b')
					print('')
					
					if eol == 1: print('\t Remember to archive your final population before your next run!')
				...
				#	while True:
				#		try:
				#			print '\n\t The current [3madjusted maximum Tree depth is:', self.tree_depth_max
				...
				#		except ValueError: print '\n\tEnter a number from 1 including 10. Try again ...'
					...
					if self.generation_id > 1:
						...
						while True:
							try:
								...
								print('\n\tTree', query, 'yields (raw):', self.algo_raw, '')
								print('\tTree', query, 'yields (sym):', self.algo_sym, '\n')
					...
					if self.generation_id == 1:
						for tree_id in range(1, len(self.population_a)):
							...
							print('\tTree', self.population_a[tree_id][0][1], 'yields (sym):', self.algo_sym, '')
							
					elif self.generation_id > 1:
						for tree_id in range(1, len(self.population_b)):
							self.fx_eval_poly(self.population_b[tree_id]) # extract the expression
							print('\tTree', self.population_b[tree_id][0][1], 'yields (sym):', self.algo_sym, '')
					...
					else: print('\n\tThe evolving population_b does not yet exist')
	...
	def fx_gen_root_node_build(self):
		...
		else: print('\n\tERROR! In fx_gen_root_node_build: pop_node_arity =', self.pop_node_arity, ''); self.fx_karoo_pause(0)
	...
	def fx_gen_child_link(self, parent_arity_sum, prior_sibling_arity, prior_siblings):
		...
		for n in range(1, len(self.tree[3]) ): # increment through all nodes (exclude 0) in array 'tree'
			...
			if int(self.tree[4][n]) == self.pop_node_depth - 1: # find all nodes that reside at the prior (parent) 'node_depth'
				...
				else: print('\n\tERROR! In fx_gen_child_link: pop_node_arity =', self.pop_node_arity, ''); self.fx_karoo_pause(0)
	...
	def fx_fitness_gym(self, population):
		...
		for tree_id in range(1, len(population)):
			...
			if self.display not in ('s'): print('\tTree', population[tree_id][0][1], 'yields (sym):', self.algo_sym, '')
			...
			if self.display == 'i':
				print('\t with fitness sum:', fitness, '\n')
	...
	def fx_fitness_tournament(self, tourn_size):
		...
		for n in range(tourn_size):
			...
			if self.fitness_type == 'max': # if the fitness function is Maximising
				...
				if fitness > tourn_test: # if the current Tree's 'fitness' is greater than the priors'
					if self.display == 'i': print('\tTree', tree_id, 'has fitness', fitness, '>', tourn_test, 'and leads')
					...
					# if int(self.population_a[tree_id][12][2]) < short_test:
						# short_test = int(self.population_a[tree_id][12][2]) # set len(algo_raw) of new leader
						# print '\twith improved parsimony score of:', short_test, ''
						
				elif fitness < tourn_test: # if the current Tree's 'fitness' is less than the priors'
					if self.display == 'i': print('\tTree', tree_id, 'has fitness', fitness, '<', tourn_test, 'and is ignored')
					# tourn_lead remains unchanged
					# tourn_test remains unchanged
					
				else: print('\n\tERROR! In fx_fitness_tournament: fitness =', fitness, 'and tourn_test =', tourn_test, ''); self.fx_karoo_pause(0)
		...
		if self.display == 'i': print('\n\tThe winner of the tournament is Tree:', tourn_winner[0][1], '')
	...
	def fx_fitness_gene_pool(self):
		...
		for tree_id in range(1, len(self.population_a)):
			...
			if len(self.population_a[tree_id][3])-1 >= self.tree_depth_min and self.algo_sym != 1: # check if Tree meets the requirements
				if self.display == 'i': print('\tTree', tree_id, 'has >=', self.tree_depth_min, 'nodes and is added to the gene pool')
		...
		if len(self.gene_pool) > 0 and self.display == 'i': print('\n\t The total population of the gene pool is', len(self.gene_pool)); self.fx_karoo_pause(0)
			...
			print("\n\t'They're dead Jim. They're all dead!' There are no Trees in the gene pool. You should archive your populations and (q)uit."); self.fx_karoo_pause(0)
	...
	def fx_fitness_test_classify(self, result):
		...
		for i in range(len(result['result'])):
			print('\tData row {} predicts class: {} ({} True)as {:.2f}{}'.format(i, int(result['labels'][0][i]), int(result['solution'][i]), result['result'][i], result['labels'][1][i]))
	...
	def fx_fitness_test_regress(self, result):
		...
		for i in range(len(result['result'])):
			print('\tData row {} predicts value: {:.2f} ({:.2f} True)'.format(i, result['result'][i], result['solution'][i]))
	...
	def fx_fitness_test_match(self, result):
		...
		for i in range(len(result['result'])):
			print('\tData row {} predicts match: {:.2f} ({:.2f} True)'.format(i, result['result'][i], result['solution'][i]))
	...
	# def fx_fitness_test_[other](self, result):
		...
		# for i in range(len(result['result'])):
			# print '\tData row {} predicts value: {} ({} label)'.format(i, int(result['result'][i]), int(result['solution'][i]))
	...
	def fx_evolve_point_mutate(self, tree):
		...
		if self.display == 'i': print('\twith', tree[5][node], 'node', tree[3][node], 'chosen for mutation\n')		
		...
		if self.display == 'db': print('\nThis is tourn_winner after node', node, 'mutation and updates:\n', tree); self.fx_karoo_pause(0)
	...
	def fx_evolve_full_mutate(self, tree, branch):
		...
		if self.display == 'db': print('\n\n[33m *** Full Mutation: function to function *** \n\nThis is the unaltered tourn_winner:\n', tree)
		...
		for n in range(len(branch)):
			...
			if tree[5][branch[n]] == 'func':
				if self.display == 'i': print('\t from', tree[5][branch[n]], 'to func ')
							
				rnd = np.random.randint(0, len(self.functions[:,0])) # call the previously loaded .csv which contains all operators
				tree[6][branch[n]] = self.functions[rnd][0] # replace function (operator)
				
			elif tree[5][branch[n]] == 'term':
				if self.display == 'i': print('\t from', tree[5][branch[n]], 'to term ')
		...
		if self.display == 'db': print('\nThis is tourn_winner after nodes', branch, 'were mutated and updated:\n', tree); self.fx_karoo_pause(0)
	...
	def fx_evolve_grow_mutate(self, tree, branch):
		...
		if branch_depth < 0: # this has never occured ... yet
			print('\n\tERROR! In fx_evolve_grow_mutate: branch_depth < 0')
			...
			if self.display == 'i': print('\tmax depth branch node', tree[3][branch_top], 'mutates from term to term\n')
			if self.display == 'db': print('\n\n[33m *** Grow Mutation: terminal to terminal *** \n\nThis is the unaltered tourn_winner:\n', tree)
			...
			if type_mod == 'term': # mutate 'branch_top' to a terminal and delete all nodes beneath (no subsequent nodes are added to this branch)
				...
				if self.display == 'i': print('\tbranch node', tree[3][branch_top], 'mutates from', tree[5][branch_top], 'to term \n')
				if self.display == 'db': print('\n\n[33m *** Grow Mutation: branch_top to terminal *** \n\nThis is the unaltered tourn_winner:\n', tree)
			...
			if type_mod == 'func': # mutate 'branch_top' to a function (a new 'gp.tree' will be copied, node by node, into 'tourn_winner')
				...
				if self.display == 'i': print('\tbranch node', tree[3][branch_top], 'mutates from', tree[5][branch_top], 'to func \n')
				if self.display == 'db': print('\n\n[33m *** Grow Mutation: branch_top to function *** \n\nThis is the unaltered tourn_winner:\n', tree)
	...
	def fx_evolve_crossover(self, parent, branch_x, offspring, branch_y):
		...
		if len(branch_x) == 1: # if the branch from the parent contains only one node (terminal)
			...
			if self.display == 'i': print('\t terminal crossover from parent', parent[0][1], 'to offspring', offspring[0][1], 'at node', branch_top, '')
			
			if self.display == 'db':
				print('\nIn a copy of one parent:\n', offspring)
			...
			if self.display == 'db': print('\nThis is the resulting offspring:\n', offspring); self.fx_karoo_pause(0)
			
		
		else: # we are working with a branch from 'parent' >= depth 1 (min 3 nodes)
		
			if self.display == 'i': print('\t branch crossover from parent', parent[0][1], 'to offspring', offspring[0][1], 'at node', branch_top, '')
			...
			if self.display == 'db':
				print('\nFrom one parent:\n', parent)
				print('\n... we copy branch_x', branch_x, 'as a new, sub-tree:\n', self.tree); self.fx_karoo_pause(0)
				
			if self.display == 'db':
				print('\n... and insert it into a copy of the second parent in place of the selected branch', branch_y,':\n', offspring); self.fx_karoo_pause(0)
	...
	def fx_evolve_branch_select(self, tree):
		...
		if self.display == 'i': print('\t with nodes', branch, 'chosen for mutation')
	...
	def fx_evolve_child_link(self, tree, node, c_buffer):
		...
		if tree[8][node] != '':
			...
			else: print('\n\tERROR! In fx_evolve_child_link: node', node, 'has arity', tree[8][node]); self.fx_karoo_pause(0)
	...
	def fx_evolve_child_insert(self, tree, node, c_buffer):
		...
		if int(tree[8][node]) == 0: # if arity = 0
			print('\n\tERROR! In fx_evolve_child_insert: node', node, 'has arity 0'); self.fx_karoo_pause(0)
		...
		else: print('\n\tERROR! In fx_evolve_child_insert: node', node, 'arity > 3'); self.fx_karoo_pause(0)
	...
	def fx_display_tree(self, tree):
		...
		print('\tTree', tree[0][1], 'yields (raw):', self.algo_raw, '')
		print('\tTree', tree[0][1], 'yields (sym):', self.algo_sym, '')
