...
class Genetic(karoo_gp.Base_GP):
    ...
    def _set_kernel(self, kernel, max_inputs):
        ...
        if kernel == 1:
            self.terminals = [random() for i in range(self.tree_pop_max)]
    ...
    def _mode1_apply(self, normalize=False):
        """ Returns a list of numbers generated by each tree's expression.
        """
        results = []
        trees = self._tree_IDs()
        
        for treeID in trees:
            result = self._symp_expr(treeID)
